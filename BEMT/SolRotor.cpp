...
double inducedVelFinderHoge(double thrust, double rho, double area, double speed) {
    int i = 0;
    double vIndOld = 999;
    double vIndNew = 5;
    double vInd;
    vIndNew = sqrt(thrust / (2 * rho * area));
    //while (fabs(vIndNew - vIndOld) > 0.01 && i < 100) {
    //    i++;
    //    vInd = vIndNew;
    //    vIndOld = vInd;
    //    vIndNew = thrust / (2 * rho * area * sqrt(pow(speed, 2) + pow(vInd, 2)));
    //    if (i == 100) {
    //        printf("100 iteration in inducedVelFinder\n");
    //    }
    //}
    return vIndNew;
}
double inducedFinderRes(double vInd, double thrust, double rho, double area, double speed, double speedtoZ) {
    // This function find the residual of induced velocity calculation. It should be used
    // with a root finding algorithm like Newton method. Modified to prevent division by 0.
    double result = 0;
    double vInflow = vInd + speedtoZ;
    result = thrust - (2 * rho * area * sqrt(speed * speed + vInflow * vInflow)) * vInd;
    return result;
}
...
void SolRotor::rotateSolRot(double input[3], double output[3]) {
    flap_rad[0] = input[0];
    flapRate_rads[0] = Vector3D(0.0, input[1], 0.0);
    double indLamb = input[2];
    Vector3D indVel(0.0, 0.0, (-indLamb * vTip_ms)); // Vector in shaft axis

    //Converting free air velocity to shaft axis with dimensions
    //Vector3D muVecSh = muVec.RotAx_X(etoshX_rad).RotAx_Y(etoshY_rad).RotAx_Z(etoshZ_rad); // Vector in SHAFT AXIS
    //Vector3D freeAirVecSh = muVecSh * (vTip_ms); // Vector in SHAFT AXIS
    //.
    ...
    output[0] = flap_rad[nOfAzimuth] - flap_rad[0];
    output[1] = flapRate_rads[nOfAzimuth].y - flapRate_rads[0].y;

    integrateForceMoment();
    double totalForceShaftY = totalLiftOnHinge.z + totalDragOnHinge.z;
    auto boundFunc = std::bind(inducedFinderRes,
        std::placeholders::_1, totalForceShaftY, density_kgm3, rotorArea_m2, sqrt(freeAirVecSh.x * freeAirVecSh.x +
            freeAirVecSh.y * freeAirVecSh.y), freeAirVecSh.z);
    Newton1D n1;
    n1.NewtonRaphson(boundFunc, 0.1);
    if (n1.status == 1) {
        output[2] = indLamb - (n1.xSol / vTip_ms);
    }
    else {
        std::cout << "The induced velocity with calculated thrust is not found" << std::endl;
    }
}
...
void SolRotor::trimRotorDummy() {
    ...
    target_pointers[0] = &(this->totalLiftOnHinge.z);
    target_pointers[1] = &(this->totalLiftOnHinge.x);
    target_pointers[2] = &(this->totalLiftOnHinge.y);
    ...
    newtonMain(&SolRotor::dummy2Method, vars_init, vars_pointers, target_pointers, targetVals, dim);
